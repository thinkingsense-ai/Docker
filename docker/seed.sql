-- Supply chain fixture, Tier B: everything in ONE real Postgres database, as four real schemas --
-- for a user who only has Postgres available (no Oracle/MySQL/SQL Server). Same story, same
-- table shapes, same seeded data, same canonical demo question/answer as the multi-database
-- fixture (../README.md) -- OmniGate still treats each schema as its own federated "backend"
-- (four BackendTarget entries, same jdbc:postgresql:// host/db, one differing only by
-- ?currentSchema=), so this genuinely exercises the same real federation code path, just with
-- zero extra infrastructure beyond one Postgres instance.
--
-- Usage: psql -h <host> -U <user> -d <db> -f seed.sql
